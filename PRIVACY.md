...
- **Storage:** the extension stores only recognition and copy preferences in local browser extension storage. The extension does not sync those preferences and does not store formulas or chat content.
...
- **数据存储：** 只在浏览器扩展的本地存储中保存识别与复制设置；扩展不会同步这些设置，也不保存公式或聊天内容。
